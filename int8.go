...
func (s *I8Slice) DeleteAll(x interface{}) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *I8Slice) I8DeleteAll(x int8) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *I8Slice) DeleteIf(f func(interface{}) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *I8Slice) I8DeleteIf(f func(int8) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
