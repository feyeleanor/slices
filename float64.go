...
func (s *F64Slice) DeleteAll(x interface{}) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *F64Slice) F64DeleteAll(x float64) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *F64Slice) DeleteIf(f func(interface{}) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *F64Slice) F64DeleteIf(f func(float64) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
