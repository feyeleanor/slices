...
func (s U16Slice) Sort()							{ sort.Sort(s) }
...
func (s *U16Slice) DeleteAll(x interface{}) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *U16Slice) U16DeleteAll(x uint16) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *U16Slice) DeleteIf(f func(interface{}) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *U16Slice) U16DeleteIf(f func(uint16) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s U16Slice) SetIntersection(o U16Slice) (r U16Slice) {
...
func (s U16Slice) SetUnion(o U16Slice) (r U16Slice) {
	cache := make(map[uint16]bool)
	for _, v := range s {
		if ok := cache[v]; !ok {
			cache[v] = true
...
func (s U16Slice) SetDifference(o U16Slice) (r U16Slice) {
	left := make(map[uint16]bool)
	right := make(map[uint16]bool)
	for _, v := range s {
		if ok := left[v]; !ok {
			left[v] = true
		}
