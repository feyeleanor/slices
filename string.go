...
func (s *SSlice) DeleteAll(x interface{}) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *SSlice) SDeleteAll(x string) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *SSlice) DeleteIf(f func(interface{}) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
...
func (s *SSlice) SDeleteIf(f func(string) bool) {
	a := *s
	p := 0
	for i, v := range a {
		if i != p {
